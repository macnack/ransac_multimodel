...
def process_patches(
    ...
    Args:
        tensor_softmax: (M, H, W) softmax-normalized heatmaps (numpy)
    ...
    out_patch_size = int(tensor_softmax.shape[0] ** 0.5)
    assert (
        out_patch_size * out_patch_size == tensor_softmax.shape[0]
    ), "Tensor's first dimension must be a perfect square."
    dict_of_gaussians = {}

    for py in range(tensor_softmax.shape[1]):
        for px in range(tensor_softmax.shape[2]):
